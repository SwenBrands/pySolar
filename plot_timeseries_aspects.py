...
variable = ['pvpot'] #variable names are harmonized to ERA5 standard by csv2nc.py
...
if time_interval == 'hour':
    time_label = '1h' #label for temporal aggregation used in output file name
else:
    raise Exception('ERROR: unknown entry for <time_label> input variable !')
...
for do in np.arange(len(domain)):
    for vv in np.arange(len(variable)):
        ...
        if dataset == 'era5_land' and variable[vv] in ('ssrd','uas','vas','tas'):
            dir_dataset = home+'/datos/OBSData/era5_land_disagg'
            file_style = 'monthly'
        elif dataset == 'era5_land' and variable[vv] == 'pvpot':
            dir_dataset = home+'/datos/OBSData/era5_land_disagg'
            file_style = 'single'
        else:
            raise Exception('ERROR: Unknown value for <dataset> and/or <variable[vv]> input parameters.')
        
        print('Plot basic times series aspects for '+variable[vv]+' from '+dataset+' at '+str(check_gridboxes)+' randomly chosen grid-boxes of the '+domain[do]+' domain, considering the time period '+str(taryears[0])+' to '+str(taryears[1]))
        dir_figs_complete = dir_figs+'/'+dataset+'/'+domain[do]+'/'+variable[vv]
        ...
        if file_style == 'yearly': #read all yearly files in <dir_dataset>
            filename_in = dir_dataset+'/'+variable[vv]+'_'+time_label+'_'+dataset+'_'+domain[do]+'*.nc'
            ...
            for yy in np.arange(len(years)):
                listdir_step = get_nc_path(years[yy],domain[do],dir_dataset,variable[vv])
            ...
            filename_in = dir_dataset+'/'+domain[do]+'/'+time_interval+'/'+variable[vv]+'/'+variable[vv]+'_'+time_label+'_'+dataset+'_'+domain[do]+'_'+str(years[0])+'0101T00_'+str(years[-1])+'1231T23.nc'
            nc_all = xr.open_dataset(filename_in)
        else:
            raise Exception('ERROR: Time series plotting options are not yet available for file_style = '+file_style+' !')

        np_var = nc_all[variable[vv]].values
        notnan = np.where((np.sum(np.isnan(np_var),axis=0)/np_var.shape[0]) != 1)
        del(np_var)
        nc_all = nc_all[variable[vv]] #convert xr dataset to data array here
        rand_indices = np.random.randint(0,high=len(notnan[0])-1,size=check_gridboxes)
        print('Plot the time series and mean daily cycle from '+str(taryears[0])+' to '+str(taryears[1])+' at '+str(check_gridboxes)+' randomly chosen grid-boxes of the '+domain[do]+' domain....')
        for gb in np.arange(len(rand_indices)):
            val_ind = rand_indices[gb]
            nc_all_point = nc_all.isel(latitude=notnan[0][val_ind],longitude=notnan[1][val_ind])
                
            #plot the hour-to-hour time series at the randomly chosen grid-box
            figure = plt.figure()
            nc_all_point.plot(linewidth=1.)
            latlabel = str(nc_all_point.latitude.values).replace('.','_')
            lonlabel = str(nc_all_point.longitude.values).replace('.','_')
            savename = dir_figs_complete+'/timeseries_'+variable[vv]+'_'+dataset+'_'+domain[do]+'_'+latlabel+'_'+lonlabel+'_'+str(years[0])+'_'+str(years[-1])+'.'+figformat
            plt.savefig(savename,dpi=dpival)
            plt.close('all')
                
            #plot the climatological mean daily cycle at the randomly chosen grid-box
            nc_clim = nc_all_point.groupby('time.hour').mean('time',skipna=True)
            figure = plt.figure()
            nc_clim.plot()
            savename = dir_figs_complete+'/mean_daily_cycle_'+variable[vv]+'_'+dataset+'_'+domain[do]+'_'+latlabel+'_'+lonlabel+'_'+str(years[0])+'_'+str(years[-1])+'.'+figformat
            plt.savefig(savename,dpi=dpival)
            plt.close('all')
            ...
            nc_clim.close()
            nc_all_point.close()
            del(nc_all_point,nc_clim)
        nc_all.close()
        del(nc_all)
    print('INFO: plot_timeseries_aspects.py has been run successfully! The new hourly output netCDF files for '+dataset+' and '+variable[vv]+' have been created in the directory '+dir_figs)
