...
taryears = [1980,1988] #start and end years used for disaggregation
    ...
    for vv in np.arange(len(variable)):
        for ti in np.arange(len(timescale)):
            ...
            if file_style == 'yearly': #read all yearly files in <dir_dataset>
                ...
                nc_all = xr.open_mfdataset(filename_in,autoclose=True)
                ...
                nc_all = xr.open_mfdataset(listdir,autoclose=True)
            ...
            for gb in np.arange(len(rand_indices)):
                ...
                savename = dir_figs_complete+'/timeseries_'+variable[vv]+'_'+timescale[ti]+'_'+dataset+'_'+domain[do]+'_'+latlabel+'_'+lonlabel+'_'+str(years[0])+'_'+str(years[-1])+'.'+figformat
                ...
                if timescale[ti] == 'hour':
                    ...
                    clim_label = 'daily_cycle'
                elif timescale[ti] == 'day':
                    nc_clim = nc_all_point.groupby('time.month').mean('time',skipna=True)
                    clim_label = 'annual_cycle'
                else:
                    raise Exception('ERROR: unknown entry for input parameter <timescale[ti]> !')

                figure = plt.figure()
                nc_clim.plot()
                savename = dir_figs_complete+'/'+clim_label+'_'+variable[vv]+'_'+timescale[ti]+'_'+dataset+'_'+domain[do]+'_'+latlabel+'_'+lonlabel+'_'+str(years[0])+'_'+str(years[-1])+'.'+figformat
                plt.savefig(savename,dpi=dpival)
                plt.close('all')
