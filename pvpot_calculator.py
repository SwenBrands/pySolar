...
for do in np.arange(len(domain)):
    ...
    for yy in np.arange(len(years)):
        ...
        pv_vals = pv.values
        print('The nan percentage, minimum, mean and maximum hourly '+varname_out+' values from '+model_dataset+' in '+str(years[yy])+' over '+domain[do]+' are:')
        print(np.round(np.sum(np.isnan(pv)).values/pv.size*100,4))
        print(np.round(np.nanmin(pv_vals),2))
        print(np.round(np.nanmean(pv_vals),2))
        print(np.round(np.nanmax(pv_vals),2))
        del(pv_vals)
        ...
        #calculate daily mean values and save
        nc_out_day = nc_out.resample(time="24H").mean()
        nc_out_day[varname_out].attrs['temporal_aggregation'] = accumulation_day
        pv_vals_day = nc_out_day[varname_out].values
        print('The nan percentage, minimum, mean and maximum daily '+varname_out+' values from '+model_dataset+' in '+str(years[yy])+' over '+domain[do]+' are:')
        print(np.round(np.nansum(np.isnan(pv_vals_day))/pv_vals_day.size*100,4))
        print(np.round(np.nanmin(pv_vals_day),2))
        print(np.round(np.nanmean(pv_vals_day),2))
        print(np.round(np.nanmax(pv_vals_day),2))
        del(pv_vals_day)
