...
taryears = [1980,1981] #start and end years used for validation
...
variable_out = 'pvpot' #variable name of the photovoltaic potential index within the output netcdf file covering the whole <taryears> period. This file is stored in the <dir_netcdf> folder.
variable_unit = 'index value from 0 to 1'
...
colormap = 'Spectral_r'
compression_level = 1
print_stats = 'yes' #print the basic statistics of the inpute and output xr arrays used and generated by this script

##EXECUTE ##############################################################
variable = ['u10','v10','ssrd','t2m'] #variable names used to calculate pv index, in fixed order i.e. the order is hard coded

#define static variables
years = np.arange(taryears[0],taryears[1]+1)
encoding = {variable_out: {'zlib': True, 'complevel': compression_level}}
for do in np.arange(len(domain)):
    #create directory of the output netcdf file if necessary
    dir_netcdf_complete_hour = dir_netcdf+'/'+domain[do]+'/'+hour_label+'/'+variable_out
    dir_netcdf_complete_day = dir_netcdf+'/'+domain[do]+'/'+day_label+'/'+variable_out
    ...
    #create one output file per year containing the index requested in variable_out
    for yy in np.arange(len(years)):
        print('Calculating PV potential for '+str(years[yy])+'...')
        listdir_u10 = get_nc_path(years[yy],domain[do],dir_rean,hour_label,variable[0])
        listdir_v10 = get_nc_path(years[yy],domain[do],dir_rean,hour_label,variable[1])
        listdir_ssrd = get_nc_path(years[yy],domain[do],dir_rean,hour_label,variable[2])
        listdir_t2m = get_nc_path(years[yy],domain[do],dir_rean,hour_label,variable[3])
        ...
        tcell = nc_t2m.values*0.943 + nc_ssrd.values*0.028 - nc_si10.values*1.528 + 4.3 #as defined in doi:10.1016/j.energy.2006.12.006 on page 1728
        ...
        pv = pr*nc_ssrd/1000 #as defined in doi:10.1016/j.energy.2006.12.006 on page 6, returns an xarray data array 
        pv = pv.rename(variable_out)
        
        #set variable attributes, pv is a xarray data array
        accum_meta_hour,accum_meta_day = get_temporal_aggregation_metadata(variable_out,variable_unit,accumulation)
        pv.attrs['standard_name'] = variable_out
        pv.attrs['long_name'] = 'photovoltaic potential index under all-sky conditions'
        pv.attrs['units'] = variable_unit
        pv.attrs['temporal_aggregation'] = accum_meta_hour
        ...
        nc_out[variable_out] = pv
        nc_out.attrs['domain'] = domain[do]
        nc_out.attrs['source'] = model_dataset+' from Copernicus Climate Data Store'
        ...
        #metadata must be assigned once again to nc_out_day
        nc_out_day.attrs = nc_out.attrs
        nc_out_day[variable_out].attrs = nc_out[variable_out].attrs 
        nc_out_day[variable_out].attrs['temporal_aggregation'] = accum_meta_day #overwrite detailed temporal aggregation metadata with the metadata generated above for daily values
        
        if file_style == 'yearly': #save to year-to-year files
            dir_hour = dir_netcdf_complete_hour
            dir_day = dir_netcdf_complete_day
            file_hour = variable_out+'_'+hour_label+'_'+model_dataset+'_'+domain[do]
            file_day = variable_out+'_'+day_label+'_'+model_dataset+'_'+domain[do]
            xr_ds_to_netcdf(nc_out,nc_out_day,encoding,file_style,dir_hour,dir_day,file_hour,file_day)
            
            # #save hourly data
            # start_date_hour = str(nc_out.time.values[0]).replace('-','').replace(':','')[0:-14]
            # end_date_hour = str(nc_out.time.values[-1]).replace('-','').replace(':','')[0:-14]
            # savename_hour = dir_netcdf_complete_hour+'/'+variable_out+'_'+hour_label+'_'+model_dataset+'_'+domain[do]+'_'+start_date_hour+'_'+end_date_hour+'.nc'
            # nc_out.to_netcdf(savename_hour,encoding=encoding)
            # #save daily data
            # start_date_day = str(nc_out_day.time.values[0]).replace('-','').replace(':','')[0:-14]
            # end_date_day = str(nc_out_day.time.values[-1]).replace('-','').replace(':','')[0:-14]
            # savename_day = dir_netcdf_complete_day+'/'+variable_out+'_'+day_label+'_'+model_dataset+'_'+domain[do]+'_'+start_date_day+'_'+end_date_day+'.nc'
            # nc_out_day.to_netcdf(savename_day,encoding=encoding)
            
        elif file_style == 'monthly': #save month-to-month files in yearly directories
            #define paths of the directories where the output netCDF data will be saved
            dir_hour = dir_netcdf_complete_hour+'/'+str(years[yy])
            dir_day = dir_netcdf_complete_day+'/'+str(years[yy])
            #generate yearly output directories if necessary
            ...
            # dates_hour = pd.DatetimeIndex(nc_out.time)
            # dates_day = pd.DatetimeIndex(nc_out_day.time)
            # months = np.unique(dates_day.month)
            # for mo in np.arange(len(months)):
                ...
                # nc_month_hour = nc_out.isel(time=monthind)
                # start_date_hour = str(nc_month_hour.time.values[0]).replace('-','').replace(':','')[0:-14]
                # end_date_hour = str(nc_month_hour.time.values[-1]).replace('-','').replace(':','')[0:-14]
                # savename_hour = dir_year_hour+'/'+variable_out+'_'+hour_label+'_'+model_dataset+'_'+domain[do]+'_'+start_date_hour+'_'+end_date_hour+'.nc'
                # nc_month_hour.to_netcdf(savename_hour,encoding=encoding)
                # nc_month_hour.close()
                # del(nc_month_hour)
                
                # #save daily data
                # monthind = np.where(dates_day.month == months[mo])[0]
                # nc_month_day = nc_out_day.isel(time=monthind)
                # start_date_day = str(nc_month_day.time.values[0]).replace('-','').replace(':','')[0:-14]
                # end_date_day = str(nc_month_day.time.values[-1]).replace('-','').replace(':','')[0:-14]
                # savename_day = dir_year_day+'/'+variable_out+'_'+hour_label+'_'+model_dataset+'_'+domain[do]+'_'+start_date_day+'_'+end_date_day+'.nc'
                # nc_month_day.to_netcdf(savename_day,encoding=encoding)
                # nc_month_day.close()
                # del(nc_month_day)
        else:
            raise Exception('ERROR: unknown entry for <file_style> input parameter !')
        ...
        if print_stats == 'yes':
            ...
            print_array_stats(pv,'The basis statistics for '+hour_label+' '+variable_out+' data from '+model_dataset+' in '+str(years[yy])+' over '+domain[do]+' are:')
            print_array_stats(nc_out_day[variable_out],'The basis statistics for '+day_label+' '+variable_out+' data from '+model_dataset+' in '+str(years[yy])+' over '+domain[do]+' are:')
    ...
    print('The output files for '+variable_out+' from '+model_dataset+' over the '+domain[do]+' domain and period '+str(years[0])+' to '+str(years[-1])+' have been stored in '+dir_netcdf+'/'+domain[do])
