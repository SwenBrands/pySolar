...
domain = ['Canarias','Iberia']
time_interval = 'hour' #temporal interval of the input and output data are
...
for do in np.arange(len(domain)):
    ...
    dir_netcdf_complete = dir_netcdf+'/'+domain[do]+'/'+time_interval+'/'+varname_out
    if os.path.isdir(dir_netcdf_complete) != True:
        os.makedirs(dir_netcdf_complete)
    ...
    for yy in np.arange(len(years)):
        ...
        nc_ssrd = nc_ssrd[variable[2]] #converts ERA5-Land data form hourly accumulation in W/m2 to W/m2 (per second) as required for pv index calculation; see in doi:10.1016/j.energy.2006.12.006
        print('The nan percentage, minimum, mean and maximum hourly '+variable[2]+' values from '+model_dataset+' in '+str(years[yy])+' over '+domain[do]+' are:')
        ...
        #set global attributesm; nc_out is a xarray dataset
        ...
        savename = dir_netcdf_complete+'/'+varname_out+'_'+time_label+'_'+model_dataset+'_'+domain[do]+'_'+start_date+'_'+end_date+'.nc'
        nc_out.to_netcdf(savename,encoding=encoding)
        ...
        pv.close()
        nc_out.close()
        del(nc_si10,nc_ssrd,nc_t2m,pv,nc_out)
    ...
    # filename_pvpot_in = aux_dir+'/pvpot_'+model_dataset+'_'+domain[do]+'*.nc'
    ...
    # nc_pv.attrs['funding'] = 'This research work was funded by the Ministry for the Ecological Transition and the Demographic Challenge (MITECO) and the European Commission NextGenerationEU (Regulation EU 2020/2094), through CSICs Interdisciplinary Thematic Platform Clima (PTI-Clima).'
    # start_date = str(nc_pv.time.values[0]).replace('-','').replace(':','')[0:-14]
    # end_date = str(nc_pv.time.values[-1]).replace('-','').replace(':','')[0:-14]
    # savename_pvpot_out = dir_netcdf_complete+'/'+varname_out+'_'+time_label+'_'+model_dataset+'_'+domain[do]+'_'+start_date+'_'+end_date+'.nc'
    # nc_pv.to_netcdf(savename_pvpot_out,encoding=encoding)
    # nc_pv.close()
    # del(nc_pv)
    print('The output files for '+varname_out+' from '+model_dataset+' over the '+domain[do]+' domain and period '+str(years[0])+' to '+str(years[-1])+' have been stored in '+savename)
