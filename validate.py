...
model_dataset = ['era5_land','era5'] #era5 or era5_land
...
dir_obs = home+'/datos/tareas/proyectos/pticlima/radiation/results/data' #path to input netcdf files produced with csv2nc.py containg AEMET station data.
...
for dd in range(len(model_dataset)):
    if model_dataset[dd] == 'era5_land':
        filename_mod = 'rsds_day_'+model_dataset[dd]+'_nn_aemet_20171201_20221230.nc'
    elif model_dataset[dd] == 'era5':
        filename_mod = 'rsds_day_'+model_dataset[dd]+'_nn_aemet_20171201_20221231.nc'
    else:
    ...
    source_obs = nc_obs[variable_h].source_short
    ...
    results = xr.merge((bias,relbias,mae,mape,rmse,pearson_r,pearson_pval_effn,spearman_r,spearman_pval_effn)) #merge xr dataarrays into a single xr dataset
    ...
    results.attrs['reference_observations'] = source_obs
    ...
    savename_results = dir_netcdf+'/verification_results_day_'+model_dataset[dd]+'_vs_aemet_corroutlier_'+corr_outlier+'_'+start_time+'_'+end_time+'.nc'
